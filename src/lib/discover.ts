...
export type { Suggestion, SuggestionKind } from './suggest';

/**
 * Discover's suggestions: businesses, places and categories.
 *
 * The ranking and de-duplication live in lib/suggest.ts so Promote and Create
 * behave identically. This only says what Discover is searching.
 */
export function searchSuggestions(
  ...
  return buildSuggestions<DiscoverBusiness>({
    items: list,
    name: (b) => b.name,
    detail: (b) => [b.category, b.location].filter(Boolean).join(' · ') || undefined,
    href: (b) => businessHref(b),
    categories: availableCategories(list),
    places,
    limit,
  }, query);
}
