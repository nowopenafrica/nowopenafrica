        ...
        <div className="space-y-5 min-w-0">
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
            ...
            <button onClick={() => copy("Yemzo is the founder of NowOpen Africa, building the operating system that gets every African business found, trusted and profitable.", 'Founder bio')}
              className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-3 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition min-h-[44px]">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button aria-label={`Mark ${n.headline} ${n.status === 'published' ? 'draft' : 'published'}`} onClick={() => void toggleStatus(n.id)}
                        className={`inline-flex items-center px-2 rounded-full text-[9px] font-bold uppercase tracking-wide hover:opacity-80 transition ${STATUS_TONE[n.status]} min-h-[44px]`}>
                        {PRESS_STATUS_LABELS[n.status]}
                      </button>
                      <button aria-label={`Remove ${n.headline}`} onClick={() => void remove(n.id)} className="inline-flex items-center justify-center w-[44px] h-[44px] rounded-lg text-gray-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500"><Trash2 size={13} /></button>
                    </div>
                  </div>
