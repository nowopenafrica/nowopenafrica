    ...
    <div className="animate-fadeIn space-y-8">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
            {looks.map((src, i) => (
              <button key={i} onClick={() => setLightbox(i)} className="aspect-square rounded-xl overflow-hidden group">
                <SmartImg src={src} alt={`Look ${i + 1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
              </button>
            ))}
          </div>
        </section>
          ...
          <div className="flex flex-wrap gap-4">
            {stylists.map((s) => (
              <div key={s.name} className="flex items-center gap-3">
                {s.image ? (
                  <SmartImg src={s.image} alt={s.name} className="w-12 h-12 rounded-full object-cover" />
                ) : (
          ...
          <ul className="space-y-3">
            {list.map((t) => (
              <li key={t.id} className="flex items-center gap-3 p-2.5 rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800">
                {t.image ? (
                  <SmartImg src={t.image} alt={t.name} className="w-16 h-16 rounded-lg object-cover flex-shrink-0" />
                ) : (
      ...
      {lightbox !== null && looks[lightbox] && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={() => setLightbox(null)}>
          ...
          <SmartImg src={looks[lightbox]} alt="Look" className="relative max-h-[85vh] max-w-full rounded-xl object-contain" onClick={(e) => e.stopPropagation()} />
