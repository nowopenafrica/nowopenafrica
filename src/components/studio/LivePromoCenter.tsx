        ...
        <div className="lg:col-span-1 rounded-2xl bg-gradient-to-br from-purple-600 to-blue-600 p-5 text-white">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <button onClick={useSuggestion} className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium bg-white text-purple-700 px-3 py-1.5 rounded-lg hover:bg-purple-50 transition">
      ...
      {showForm && (
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {CHANNELS.map((c) => (
                <button key={c} onClick={() => setChannels((prev) => prev.includes(c) ? prev.filter((x) => x !== c) : [...prev, c])}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition ${channels.includes(c) ? 'bg-purple-600 text-white' : 'border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300'}`}>
                  {c}
                </button>
              ))}
            </div>
          ...
          <div className="flex justify-end">
            <button onClick={add} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium bg-purple-600 text-white hover:bg-purple-700 transition">
            ...
            return (
              <div key={p.id} className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {st === 'scheduled' && (
                      <button onClick={() => setLive(p.id)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-green-600 text-white hover:bg-green-700 transition">
                        <CheckCircle2 size={13} /> Go live
                      </button>
                    )}
                    {st === 'live' && (
                      <button onClick={() => end(p.id)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                        <Clock size={13} /> End
                      </button>
                    )}
                    <button onClick={() => share(p)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-blue-600 text-white hover:bg-blue-700 transition">
                      <Send size={13} /> WhatsApp
                    </button>
                    <button onClick={() => onNavigate('promotions')} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-purple-600 text-white hover:bg-purple-700 transition">
                      <WalletCards size={13} /> Design asset
                    </button>
                    <button onClick={() => remove(p.id)} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium text-gray-400 hover:text-red-500 transition">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
